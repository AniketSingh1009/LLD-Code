...
public class ObserverDesignPattern {
    public static void main(String[] args) {
        ...
        Subscriber subs3 = new Subscriber("Ravi", channelB);
        Subscriber subs4 = new Subscriber("Sita", channelB);

        // Varun and Tarun subscribe to CoderArmy
        channel.subscribe(subs1);
        channel.subscribe(subs2);
        channelB.subscribe(subs3);
        channelB.subscribe(subs4);
        channelB.subscribe(subs1);

        // Upload a video: both Varun and Tarun are notified
        channel.uploadVideo("Observer Pattern Tutorial");
        channelB.uploadVideo("Comedy Skit");
        channel.unsubscribe(subs2);
        channelB.unsubscribe(subs4);
        channelB.unsubscribe(subs1);
        channelB.unsubscribe(subs3);
        // Varun unsubscribes; Tarun remains subscribed

        // Upload another video: only Tarun is notified
        channel.uploadVideo("Decorator Pattern Tutorial");
        channelB.uploadVideo("Funny Moments Compilation");
    }
}